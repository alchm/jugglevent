...
/*
 locals
 Makes current session and user available on templates with "session" and "user" accessors
 */
...
exports.registrationFormValidator = form(
    ...
    field("lastname")   .required(ID+"What is your last name ?").is(/^[a-zA-Z]+$/),
...
// EST-CE QUE METTRE REQUIRED AVANT PERMETTRAIT D'AVOIR ACCES AU MESSAGE?
...
exports.associationRegistrationFormValidator = form(
    ...
    field("name")       .required(ID+"What is your organisation name ?")    .is(/^[a-zA-Z]+$/, ID+"Your organisation name is invalid"),
    ...
    field("password")   .required(ID+"What is your user password ?")        .custom(function(value) {
        if (value.length < 6) throw new Error(ID+"Your password length must be greater than 6");
    })
);